...
export class DateProcessing {
  ...
  static correctionDate: DateProcessingProps["correctionDate"] = (date, options) => {
    ...
    if (options?.shortYear) {
      return options?.withOutCurrentYear ? DateProcessing.correctionDateWithOutCurrentYear(date, { shortYear: true }) : DateProcessing.correctionShortYear(date);
    }

    // return DateProcessing.splitDateFromTime(date)[0]//гггг-мм-дд
    return options?.withOutCurrentYear
  ...
  static getDifferenceDates: DateProcessingProps["getDifferenceDates"] = (date1, date2) => {
  ...
  */
  static calculateTimeDifference: DateProcessingProps["calculateTimeDifference"] = (startISODateTime, endISODateTime) => {
    ...
    const result = {
      ...
      formatted: `${days} дней ${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")}:${seconds.toString().padStart(2, "0")}`,
      formattedShort: `${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")}:${seconds.toString().padStart(2, "0")}`,
      days,
      hours,
      minutes,
      seconds,
  ...
  static getChunkFromDate: DateProcessingProps["getChunkFromDate"] = (date, chunk, option) => {
    ...
    switch (chunk) {
      case "day":
        return !option?.isBeforeZero ? String(parseInt(chunkDate[2])) : chunkDate[2];
      case "month":
        return !option?.isBeforeZero ? String(parseInt(chunkDate[1])) : chunkDate[1];
      case "year":
        return chunkDate[0];
    }
  };
  ...
  static minMaxMountStr: DateProcessingProps["minMaxMountStr"] = (obMinMaxDate) => {
    return {
      minMaxMonth:
        DateProcessing.getNameMonthByNumber(Number(DateProcessing.getChunkFromDate(obMinMaxDate.minDate, "month", { isBeforeZero: false }))) +
        " - " +
        DateProcessing.getNameMonthByNumber(Number(DateProcessing.getChunkFromDate(obMinMaxDate.maxDate, "month", { isBeforeZero: false }))),
    };
  };

  static getDatesToCurrentDate: DateProcessingProps["getDatesToCurrentDate"] = (periods) =>
    periods.filter((period) => DateProcessing.hasDateLessPeriod(period, DateProcessing.getCurrentDate()));
